...
//	int N; cin >> N;
//	vector<Point> v(N); rep(i, 0, N) { cin >> v[i]; }
//	debug(v);
...
//		rep(j, 0, N) {
//			if (i == j)continue;
//			args.emplace_back(arg(v[j] - v[i]));
//		}
//		sort(all(args));
//		dump(args);
//		rep(j, 0, N) {
//			if (i == j)continue;
//			auto f = [&args, &j](double x) {
//				return args[j] + x > PI / 2 + EPS;
//			};
//			int res = binary_search_(0, args.size(), f);
//			dump(args[res], args[res] + args[j]);
//		}
//	}
//	return 0;
//}
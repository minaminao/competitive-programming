...
void solve1() {
	int N; cin >> N;
	vector<Point> A(N); rep(i, 0, N) { cin >> A[i]; }
	vector<Point> B(N); rep(i, 0, N) { cin >> B[i]; }
	Segment s(A[0], A[1]);
	Polygon C = convex_hull(A), D = convex_hull(B);
	double a = diameter(C), b = diameter(D);
	cout << b / a << endl;
}
...
void solve2() {
	...
	vector<Point> A(N); rep(i, 0, N) { cin >> A[i]; }
	vector<Point> B(N); rep(i, 0, N) { cin >> B[i]; }
	Point ca = center(A), cb = center(B);
	double la = 0, lb = 0;
	rep(i, 0, N) {
		chmax(la, get_distance(ca, A[i]));
		chmax(lb, get_distance(cb, B[i]));
	}
	cout << lb / la << endl;
}

...
//		Polygon a[2];
//		int idx = 0;
//		a[0].emplace_back(x[0], y[0]);
//		rep(i, 1, n) {
//			if (g[i].x == 0) {
//				idx++;
//			}
//			g.emplace_back(x[i], y[i]);
//		}
//		a[1].emplace_back(x.front(), y.front());
//		rep(i, 0, a[1].size()) {
//			a[1][i].x = abs(a[1][i].x);
//		}
//		g.clear();
//		rep(i, 0, a[0].size()) {
//			if (!contains(a[1], a[0][i])) {
//				g.emplace_back(a[0][i]);
//			}
//		}
//		rep(i, 0, a[1].size()) {
//			if (!contains(a[0], a[1][i])) {
//				g.emplace_back(a[1][i]);
//			}
//		}
//		sort(g.begin(), g.end(), [](const Point &p1, const Point &p2) {
//			return p1.y != p2.y ? p1.y < p2.y : p1.x < p2.x;
//		});
//		auto xmax = max_element(all(g));
//		sort(g.begin(), xmax, [](const Point &p1, const Point &p2) {
//			return p1.y != p2.y ? p1.y < p2.y : p1.x < p2.x;
//		});
//		sort(xmax, g.end(), [](const Point &p1, const Point &p2) {
//			return p1.y != p2.y ? p1.y < p2.y : p1.x > p2.x;
//		});
//		int ret = 0;
//		int xmaxi = xmax - g.begin();
//		Line Y(Point(0, -200), Point(0, 200));
//		rep(i, 1, xmaxi + 1) {
//			Segment s(g[i], g[i - 1]);
//			Point cross = get_cross_point(Y, s);
//			ret += abs(PI*g[i].x*g[i].x*(cross.y - y[i]) / 3
//				- PI*g[i - 1].x*g[i - 1].x*(cross.y - y[i - 1]) / 3);
//		}
//		rep(i, 1, xmaxi + 1) {
//			Segment s(g[i], g[i - 1]);
//			Point cross = get_cross_point(Y, s);
...
//		int a = skills[0], b = skills[1];
//		queue<int> q;
//		rep(i, 2, skills.size())q.emplace(skills[i]);
//		int cnta = 0, cntb = 0;
//		rep(i, 0, K - 1) {
//			dump(a, b, q);
//			if (a < b) {
...
//	//PingPongQueue c;
//	//int A; cin >> A;
//	//vector<int> v(A); rep(i, 0, A) { cin >> v[i]; }
//	//int N, K; cin >> N >> K;
//	//auto res = c.whoPlaysNext(v, N, K);
//	//dump(res);
//
//	//int a, b, c; cin >> a >> b >> c;
//	//int s; cin >> s;
